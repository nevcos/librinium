...
import { createEmptyGistState, getDocuments } from '../../domain/documentStoreState/documentStoreStateSelectors';
...
describe("<Sidebar />", () => {
  ...
  it.each(documentContentTypeValues)("should create a %s document and select it when clicking on create %s button", async (type: DocumentContentType) => {
    ...
    const document = getDocuments(getState().gist)[0];
  ...
  it("should open document when clicking on open link for first document", async () => {
    ...
    addDocument(state.gist, {payload: DOCUMENT_0});
    addDocument(state.gist, {payload: DOCUMENT_1});
  ...
  it("should rename when double clicking on select button for first document", async () => {
    ...
    expect(getDocuments(getState().gist)[0]?.name).toBe(newName);
  });
